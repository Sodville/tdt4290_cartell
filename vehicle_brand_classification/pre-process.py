...
def is_data_present(dataset):
    if (os.path.exists(f"data/{dataset}/train") and
            len(os.listdir(f"data/{dataset}/train"))):
        return True
    if (os.path.exists(f"data/{dataset}/valid") and
            len(os.listdir(f"data/{dataset}/valid"))):
        return True
    if (os.path.exists(f"data/{dataset}/test") and
            len(os.listdir(f"data/{dataset}/test"))):
        return True
    return False
...
def save_train_data(fnames, labels, bboxes):
    ...
    pb = ProgressBar(total=100, prefix="Save train data", suffix="",
                     decimals=3, length=50, fill="=")
...
def save_test_data(fnames, bboxes):
    ...
    pb = ProgressBar(total=100, prefix="Save test data", suffix="",
                     decimals=3, length=50, fill="=")
...
def vmmr_save_data(src_folder, keep_models, class_cutoff, img_width,
                   img_height, verbose):
    if not src_folder:
        print(("No source directory for VMMR-dataset supplied, "
               "--vmmr_folder-argument required."))
        return

    labels = os.listdir(src_folder)
    num_train, num_valid, num_test = 0, 0, 0
    classes_skipped, classes_blacklisted = 0, 0

    print("Splitting VMMR dataset into a test, validation and training set.")
    ...
    for label in labels:
        ...
        if n_images < class_cutoff:
            if verbose:
                print((f"Skipping class {label} due to insufficient "
    ...
    print((f"{classes_blacklisted} classes were skipped due to brand not "
           f"being whitelisted and {classes_skipped} of classes with "
           f"whitelisted brand were skipped due to an image number less "
           f"than cutoff {class_cutoff}."))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--vmmr_folder",
                        help=("source folder containing "
                              "the entire VMMRdb-dataset"))
    parser.add_argument("--keep_models", action="store_true",
                        help=("keep models in labels "
                              "(e.g. volvo_v70, rather than just volvo)"))
    parser.add_argument("--class_cutoff", type=int, default=5,
                        help=("ignore classes that have less "
                              "images than this (default 5)"))
    parser.add_argument("-f", "--force", dest="force", action="store_true",
                        help=("run preprocessing even if processed data is "
                              "found, this might overwrite existing data"))
    ...
    if dataset == "stanford":
        ...
        vmmr_save_data(args.vmmr_folder, args.keep_models, args.class_cutoff,
